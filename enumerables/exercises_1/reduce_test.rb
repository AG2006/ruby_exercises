...
class ReduceTest < Minitest::Test
  ...
  def test_multiply_list_of_numbers
    numbers = [2, 3, 5, 7]
    result = numbers.reduce(1) { |total, number| total * number }
    assert_equal 210, result
  end

  def test_capitalize_keywords_in_phrase_one_fish_two_fish_red_fish_blue_fish
    keywords = ["fish", "blue"]
    result = keywords.reduce('one fish two fish red fish blue fish') do |result, keyword|
      result.gsub(keyword, keyword.upcase)
    end
  ...
  def test_divide_560_by_a_bunch_of_numbers
    numbers = [2, 2, 2, 5, 7]
    result = numbers.reduce(560) { |total, number| total / number}
    assert_equal 2, result
  end

  def test_subtract_smallest_values_from_100
    elements = [[8, 5, 3], [1, 9, 11], [4, 7, 2], [19, 34, 6]]
    result = elements.reduce(100) { |result, number| result - number.min }
