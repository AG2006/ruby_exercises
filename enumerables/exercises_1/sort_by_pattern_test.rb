...
class SortByPatternTest < Minitest::Test
  ...
  def test_sort_by_distance
    ...
    distances.each do |distance|
      value = distance.delete("cm").to_i
      transformed << [value, distance]
    end
    transformed = transformed.sort
    sorted = []
  ...
  def test_sort_by_proximity_to_ten
    ...
    prices.each do |price|
      difference = (10 - price).abs
      transformed << [difference, price]
    end
    transformed = transformed.sort
    sorted = []
  ...
  def test_sort_by_number_of_cents
    ...
    transformed = []
    prices.each do |price|
      remainder = price % price.to_i
      transformed << [remainder, price]
    end
    transformed = transformed.sort
    sorted = []
    transformed.each do |sort_key, price|
