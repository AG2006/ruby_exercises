...
class NestedTest < MiniTest::Test
  ...
  def test_list_of_employees_across_all_restaurants
    #=======================
    employee_names = stores.map do |restaurant, info|
      info[:employees]
    end
  ...
  def test_list_of_all_ingredients_across_all_restaurants
    #=======================
    ingredients = stores.map do |restaurant, restaurant_info|
      restaurant_info[:dishes].map do |dish_info|
  ...
  def test_full_menu_price_for_olive_garden
    #=======================
    full_menu_price = stores[:olive_garden][:dishes].reduce(0) do |sum, item|
      sum + item[:price]
    end
    #=======================
  ...
  def test_menu_accross_all_restaurants
    #=======================
     full_menu = stores.reduce({}) do |collection, store|
       store[1][:dishes].each do |dish|
         collection[dish[:name]] = dish
       end
       collection
     end
    #=======================
